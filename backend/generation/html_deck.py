...
# ─── EXTRACTION PROMPT ─────────────────────────────────────────────────────
# Prompt text lives in agents/wireframe_designer_agent/SKILL.md (single source of
# truth, kept in sync with the slide schemas this module renders). This module only
# configures HOW it's called (model, temperature, max_tokens) — same pattern other
# skills use for their SKILL.md. Loaded once at import time and fails loudly if the
# file is missing/empty, so a bad path can never silently degrade extraction quality
# (e.g. by falling back to an empty prompt).
_SKILL_MD_PATH = os.path.normpath(
    os.path.join(
        os.path.dirname(os.path.abspath(__file__)),
        "..", "agents", "wireframe_designer_agent", "SKILL.md",
    )
)
...
def _load_extract_system(path: str) -> str:
    if not os.path.exists(path):
        raise FileNotFoundError(
            f"[HTMLDeck] wireframe_designer extraction prompt not found at {path!r}. "
            "Slide extraction cannot run without it — check that "
            "agents/wireframe_designer_agent/SKILL.md exists."
        )
    with open(path, "r", encoding="utf-8") as f:
        content = f.read()
    if not content.strip():
        raise ValueError(f"[HTMLDeck] wireframe_designer extraction prompt at {path!r} is empty.")
    return content
...
class HTMLDeckGenerator:
    """Generates self-contained AdtimaBox HTML slide decks per agents/wireframe_designer_agent/SKILL.md."""
