...
class Brief(BaseModel):
    ...
    """
    industry: Optional[str] = Field(None, description="Industry sector (e.g., F&B, Retail, Tech)")
    ...
    specific_requirements: Optional[list[str]] = Field(default_factory=list, description="Specific requirements")
    constraints: Optional[list[str]] = Field(default_factory=list, description="Known constraints")
...
class Question(BaseModel):
    ...
    is_mandatory: bool = Field(False, description="Whether this question must be answered to proceed")
    assumption: Optional[str] = Field(None, description="What will be assumed if optional question is skipped")
    target_field: str = Field(..., description="Which brief field this question populates")
    asked_count: int = Field(0, description="Number of times this question has been asked")
    last_asked_at: Optional[datetime] = Field(None, description="Last time this question was asked")
    ...
    was_helpful: Optional[bool] = Field(None, description="Feedback: was this question helpful")
...
class NeedsRequest(BaseModel):
    ...
    context: dict[str, Any] = Field(default_factory=dict, description="Context to pass to the requested agent")
...
class AgentOutput(BaseModel):
    ...
    payload: dict[str, Any] = Field(default_factory=dict, description="The actual result data")
    ...
    questions: list[Question] = Field(default_factory=list, description="Questions to ask user")
...
class ValidationReport(BaseModel):
    ...
    ambiguities: list[Ambiguity] = Field(
        default_factory=list,
        description="Fields with ambiguous values"
    )
    kb_confidence: float = Field(
        1.0, ge=0.0, le=1.0,
        description="Knowledge base retrieval confidence"
    )
...
class AgentTask(BaseModel):
    ...
    is_critical: bool = Field(True, description="Whether failure should halt the pipeline")
...
class CheckpointAction(BaseModel):
    ...
    type: Literal["generate_pptx", "generate_wireframe", "generate_userflow",
                    "generate_quote", "send_external", "other"] = Field(
        ..., description="Type of action"
    )
    description: str = Field(..., description="Human-readable description")
    parameters: dict[str, Any] = Field(default_factory=dict, description="Action parameters")
    preview: Optional[dict[str, Any]] = Field(None, description="Preview of what will be generated")
...
class Checkpoint(BaseModel):
    ...
    created_at: datetime = Field(default_factory=datetime.now)
    updated_at: datetime = Field(default_factory=datetime.now)
    decided_at: Optional[datetime] = Field(None, description="When user made a decision")
...
class FeedbackRule(BaseModel):
    ...
    source_quote: str = Field(..., description="What the user said that triggered this rule")
    created_at: datetime = Field(default_factory=datetime.now)
...
class ProfileHistoryItem(BaseModel):
    ...
    outcome: Optional[Literal["won", "lost", "pending"]] = Field(None, description="Case outcome")
...
class SalespersonProfile(BaseModel):
    ...
    constraints: list[str] = Field(
        default_factory=list,
        description="Active feedback rule IDs"
    )
...
class SalesCaseState(BaseModel):
    ...
    validation_status: Literal["PENDING", "READY", "BLOCKED"] = Field(
        "PENDING",
        description="Whether brief is ready for agent dispatch"
    )
    ...
    question_stack: list[Question] = Field(
        default_factory=list,
        description="Stack of questions to ask user"
    )

    # Execution
    plan: Optional[ExecutionPlan] = Field(
        None,
        description="Execution plan for agents"
    )
    ...
    visited: list[str] = Field(
        default_factory=list,
        description="Agents already visited in this workflow"
    )
    hop_depth: int = Field(
        0,
        description="Current depth in the agent workflow"
    )

    # Memory & learning
    profile: Optional[SalespersonProfile] = Field(
        None,
        description="Salesperson profile"
    )
    constraints: list[FeedbackRule] = Field(
        default_factory=list,
        description="Active feedback rules"
    )
    ...
    messages: list[dict[str, Any]] = Field(
        default_factory=list,
        description="Message history for this session"
    )
    summary: str = Field(
        "",
        description="Rolling summary of the conversation"
    )
