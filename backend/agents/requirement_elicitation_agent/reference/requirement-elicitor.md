...
Work through 6 layers in order. Start with Layer 0 (AS-IS) before proposing anything. Prioritize Layer 0 and Layer 1, but do not cap the number of questions — ask everything needed to reach a feasibility verdict, in one grouped turn.
...
- Lead with the most blocking unknowns, but do not cap the count — ask everything needed to reach a feasibility verdict, grouped into one turn with a reason per question
