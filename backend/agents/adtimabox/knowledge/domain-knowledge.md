...
## PATTERN 12: ZNS MESSAGE AUTOMATION CADENCE FOR PHARMA EVENTS

**Scenario:** Pharma brand wants to automate Zalo message touchpoints at every stage of a medical event (online or offline) — from pre-registration to post-event follow-up.

**The 4-message event cadence (triggered automatically):**

| Label | Trigger | Message purpose | Key content |
|---|---|---|---|
| M1 | HCP completes Hub registration | Welcome to Hub | Welcome, explain Hub features, next steps |
| M2 | HCP registers for specific event | Event registration confirmed | Event details (date, time, venue/link), unique check-in QR code (offline) or meeting link (online) |
| M3 | Before event day (configurable: D-1, D-day morning) | Event reminder + pre-survey | Remind logistics + invite to fill pre-event survey |
| M4 | After event (triggered by event end time) | Recap + post-event survey | Thank-you, recap content/recording link, post-event survey |
