...
description: >
    ...
    AdtimaBox solution architect — activate after BA elicitation when requirements are clear and agent needs to design the actual user journey. Queries miniapp-specialist for core flows, compares with client's desired flow, identifies gaps, then outputs a complete journey with core vs custom flags. Input: constraint map or clear client requirement. Output: full journey flow document with each step labeled core or custom + cost flags. Triggers: "design the flow", "what's the journey for this client", "map out the solution".
